...
class MeshModule : RCTEventEmitter{
  ...
  override func supportedEvents() -> [String]! {
    return ["onNetworkLoaded", "onScanResult", "onAppKeyUpdated", "onNodeConnected", "onNodeIdentified","onStateChange", "onProgressUpdate", "onProvisionCompleted", "onProvisionedScanResult", "onNetworkKeyUpdated", "onModelKeyUpdated", "onPublicationUpdated", "onSubscriptionReceived", "onSubscriptionAdded", "onSubscriptionFailed", "onSensorGet", "onStatusReceived", "onModelKeyUpdated", "onProxyFilterUpdated", "onBindAppKeysDone", "onSubscriptionDone", "onPublicationDone"]
  }
  ...
  func getMeshNetworkName(_ callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      ...
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "No network loaded"]), NSNull()])
    }
  }
  ...
  func getProvisionedMeshNodes(_ callback: @escaping RCTResponseSenderBlock) {
    guard let network = self.meshRepository.getMeshNetwork() else {
      let error = NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mesh network is null"])
    ...
    for node in network.nodes {
      ...
      if node.features?.friend != .notSupported {
        featuresArray.append("F")
      }
      if node.features?.proxy != .notSupported {
        featuresArray.append("P")
      }
      if node.features?.lowPower != .notSupported {
        featuresArray.append("LP")
      }
      if node.features?.relay != .notSupported {
        featuresArray.append("R")
      }
      nodeData["features"] = featuresArray
  ...
  func getMeshNetworkTimestamp(_ callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      ...
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "getMeshNetworkTimestamp"]), NSNull()])
    }
  }
  ...
  func removeNetworksKey(_ keyIndex: Int, callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      ...
      if let keyToRemove = network.networkKeys.first(where: { $0.index == keyIndex }) {
        do {
          ...
          callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: error.localizedDescription]), NSNull()])
        }
      } else {
        callback([NSError(domain: "MeshModule", code: 1, userInfo: [NSLocalizedDescriptionKey: "Key not found"]), NSNull()])
      }
    } else {
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mesh network not found"]), NSNull()])
    }
  }
  ...
  func removeAppKey(_ keyIndex: Int, callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      // Find the key to remove
      if let keyToRemove = network.applicationKeys.first(where: { $0.index == keyIndex }) {
        do {
          // Attempt to remove the key, which can throw an error
  ...
  @objc
  func editNetworksKey(_ keyIndex: Int, newName: String, newHexKeyString: String?, callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      ...
      if let keyToEdit = network.networkKeys.first(where: { $0.index == keyIndex }) {
        ...
        if let newHexKeyString = newHexKeyString {
          ...
          guard newNetworkKeyData?.count == 16 else {
            callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid hex string for network key"]), NSNull()])
            return
          }
        }
        ...
        do {
          ...
          if let newNetworkKeyData = newNetworkKeyData {
            ...
            try network.add(networkKey: newNetworkKeyData, withIndex:  UInt16(keyIndex), name: newName)
          ...
          callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Error editing network key"]), NSNull()])
        }
      } else {
        callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Network key not found"]), NSNull()])
      }
    } else {
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mesh network not found"]), NSNull()])
    }
  ...
  @objc
  func editAppKey(_ keyIndex: Int, newName: String, newHexKeyString: String?, callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      // Find the existing network key by its index
      if let keyToEdit = network.applicationKeys.first(where: { $0.index == keyIndex }) {
        ...
        if let newHexKeyString = newHexKeyString {
          ...
          // Ensure the new key is valid
          guard newNetworkKeyData?.count == 16 else {
            callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid hex string for app key"]), NSNull()])
            return
          }
        }
        ...
        do {
          ...
          if let newNetworkKeyData = newNetworkKeyData {
            ...
            // Recreate the key with the new key value and name
            try network.add(applicationKey: newNetworkKeyData, withIndex:  UInt16(keyIndex), name: newName)
            self.meshRepository.onNetworkUpdated();
          } else {
            // If no new key is provided, just update the name
            keyToEdit.name = newName
          ...
          callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Error editing app key"]), NSNull()])
        }
      } else {
        callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "App key not found"]), NSNull()])
      }
    } else {
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mesh network not found"]), NSNull()])
    }
  ...
  @objc
  func addNetworksKey(_ name: String, hexKeyString: String, callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      // Convert hex string to Data
      guard let networkKeyData = Data(hexString: hexKeyString as String) else {
        // Return error if hex string is invalid
        callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid hex key string"]), NSNull()])
        return
      }
      
      // Add network key to mesh network
      do {
        let networkKey = try network.add(networkKey: networkKeyData, name: name as String)
        ...
        callback([NSError(domain: "MeshModule", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to add network key: \(error.localizedDescription)"]), NSNull()])
      }
    } else {
      // Return error if mesh network is not found
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mesh network is not available"]), NSNull()])
    }
  }
  
  @objc
  func addAppKey(_ name: String, hexKeyString: String, callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      // Convert hex string to Data
      ...
      // Add network key to mesh network
      do {
        let appKey = try network.add(applicationKey: appKeyData, name: name as String)
        // Return success and the added network key details
        var keyDict: [String: Any] = [:]
        keyDict["name"] = appKey.name
        keyDict["key"] = appKey.key.hex
        ...
        // Handle any errors that occur while adding the network key
        callback([NSError(domain: "MeshModule", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to add app key: \(error.localizedDescription)"]), NSNull()])
      }
    } else {
      // Return error if mesh network is not found
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mesh network is not available"]), NSNull()])
  ...
  @objc
  func setMeshNetworkName(_ newNetName: String, callback: RCTResponseSenderBlock) {
    if let network = self.meshRepository.getMeshNetwork() {
      ...
      do {
        ...
        callback([NSError(domain: "MeshModule", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to change net name \(error.localizedDescription)"]), NSNull()])
      }
    } else {
      // Return error if mesh network is not found
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mesh network is not available"]), NSNull()])
  ...
  @objc
  func selectUnprovisionedNode(_ nodeId: String, callback: @escaping RCTResponseSenderBlock) {
    // Loop through scan results and find the matching peripheral
    for result in self.meshRepository.getScanResults() {
      if result.device.uuid == UUID(uuidString: nodeId) {
        // Try to connect to the peripheral
        self.meshRepository.selectUnprovisionedNode(unprovisionedDevice: result);
        return
      }
    }
    ...
    callback([NSError(domain: "MeshModule", code: 1, userInfo: [NSLocalizedDescriptionKey: "Peripheral with nodeId \(nodeId) not found."]), NSNull()])
  }
  
  @objc
  func selectProvisionedNodeToConnect(_ nodeId: String, unicastAddress: Int , callback: @escaping RCTResponseSenderBlock) {
    self.meshRepository.bleManager.setSelectedProvisionedDevice(peripheralUuid: UUID(uuidString: nodeId)!, nodeUnicastAddress: unicastAddress)
    
    self.meshRepository.connectToProvisionedNode(nodeId: nodeId, nodeUnicastAddress: unicastAddress)
  ...
  func identifyNode(_ callback:  RCTResponseSenderBlock) {
    do {
      ...
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to identify node: \(error.localizedDescription)"]), NSNull()])
    }
  }
  
  @objc
  func exportNetwork(_ callback:  RCTResponseSenderBlock) {
    guard let exportedData = self.meshRepository.getMeshManager()?.export() else {
      callback(["Failed to export network"])
      return
    }
    
    do {
      // Decode the exported JSON data to a Dictionary to manipulate formatting
      let jsonObject = try JSONSerialization.jsonObject(with: exportedData, options: [])
  ...
  @objc
  func getProvisionedNodeNetworkKeys(_ unicastAddress: Int, callback: RCTResponseSenderBlock) {
    do {
      // Call the repository function to get network keys
      guard let networkKeys = self.meshRepository.getProvisionedNodeNetworkKeys(unicastAddress: unicastAddress) else {
        callback(["No network keys found for the given unicast address", NSNull()])
        return
      }
  ...
  @objc
  func getProvisionedNodeAppKeys(_ unicastAddress: Int, callback: @escaping RCTResponseSenderBlock) {
    
    guard let node = AppDelegate.shared.meshNetworkManager.meshNetwork?.node(withAddress: Address(unicastAddress)) else {
  ...
  @objc
  func addNodeNetworkKeys(_ unicastAddress: Int, keyIndex: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.addNodeNetworkKeys( unicastAddress:unicastAddress, keyIndex:keyIndex) { success, error in
      if success {
        print("addNodeNetworkKeys successfully to key \(keyIndex) for node \(unicastAddress)")
        self.meshRepository.onNetworkUpdated()
        ...
        self.sendEvent(withName: "onNetworkKeyUpdated", body: error?.localizedDescription)
        
        print("Failed to addNodeAppKey for node \(unicastAddress): \(errorMessage)")
  ...
  @objc
  func removeNodeNetworkKeys(_ unicastAddress: Int, keyIndex: Int, callback:  @escaping RCTResponseSenderBlock){
    meshRepository.removeNodeNetworkKeys(unicastAddress: unicastAddress, keyIndex: keyIndex) { success, error in
      if success {
        print("removeNodeNetworkKeys successfully to key \(keyIndex) for node \(unicastAddress)")
        self.meshRepository.onNetworkUpdated()
        
        self.sendEvent(withName: "onNetworkKeyUpdated", body: "success")
        
        callback([NSNull(), ["success": true, "removeNodeNetworkKeys": keyIndex]])
      } else {
        let errorMessage = error?.localizedDescription ?? "Unknown error"
        print("Failed to removeNodeNetworkKeys for node \(unicastAddress): \(errorMessage)")
        callback([["error": errorMessage], NSNull()])
      }
    }
  ...
  func isDeviceConnected(_ unicastAddress: Int, callback: RCTResponseSenderBlock){
    guard let connectedDevice = self.connectedDevices.first(where: {
      $0.nodeUnicastAddress == unicastAddress
    }) else {
      callback([NSNull(), false])
      return
    }
    let isConnected = meshRepository.isDeviceConnected(peripheralUUID: connectedDevice.uuid)
    callback([NSNull(), isConnected])
  }
  
  @objc
  func getProvisionedNode(_ unicastAddress: Int, callback: @escaping RCTResponseSenderBlock) {
    guard let meshNetwork = meshRepository.getMeshNetwork() else {
      callback(["Mesh network not initialized", NSNull()])
      return
    }
    
    guard let node = meshNetwork.node(withAddress: UInt16(unicastAddress)) else {
      callback(["Node not found", NSNull()])
      return
    }
    
    if let connectedDevice = self.connectedDevices.first(where: { $0.nodeUnicastAddress == unicastAddress }) {
      if meshRepository.isDeviceConnected(peripheralUUID: connectedDevice.uuid) {
        self.meshRepository.bleManager.setSelectedProvisionedDevice(peripheralUuid: connectedDevice.uuid, nodeUnicastAddress: unicastAddress)
      }
    }
    
    meshRepository.getTtl(unicastAddress: unicastAddress) { result, error in
      var ttl = UInt8(255)
      
      if let result = result {
        ttl = result
      } else {
        print("FAILED TO GET TTL")
      }
      ...
      for element in node.elements {
        ...
        for model in element.models {
          ...
          modelData["type"] = model.isBluetoothSIGAssigned ? "Bluetooth SIG" : (model.companyIdentifier != nil ? CompanyIdentifiers.name(for: UInt16(model.companyIdentifier!)) ?? "Unknown" : "Unknown")
  ...
  @objc
  func removeNodeFromNetwork(_ unicastAddress: Int, callback: RCTResponseSenderBlock){
    if let node = meshRepository.getMeshNetwork()?.node(withAddress: UInt16(unicastAddress)){
  ...
  @objc
  func editProvisionedNodeName(_ unicastAddress: Int,nodeName: String, callback: RCTResponseSenderBlock){
    if let node = meshRepository.getMeshNetwork()?.node(withAddress: UInt16(unicastAddress)){
      node.name = nodeName;
      meshRepository.onNetworkUpdated()
  ...
  @objc
  func editProvisionedNodeTtl(_ unicastAddress: Int, newTtl: Int, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.setTtl(unicastAddress: unicastAddress, ttl: newTtl) { success, error in
      if (success != nil) {
  ...
  @objc
  func addNodeAppKey(_ unicastAddress: Int, keyIndex: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.addNodeAppKey( unicastAddress:unicastAddress, keyIndex:keyIndex){ success, error in
      if success {
        print("addNodeAppKey successfully to key \(keyIndex) for node \(unicastAddress)")
        self.meshRepository.onNetworkUpdated()
        ...
        let errorMessage = error?.localizedDescription ?? "Unknown error"
        self.sendEvent(withName: "onAppKeyUpdated", body: error?.localizedDescription)
  ...
  @objc
  func removeNodeAppKeys(_ unicastAddress: Int, keyIndex: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.removeNodeAppKeys( unicastAddress:unicastAddress, keyIndex:keyIndex){ success, error in
      if success {
        print("addNodeAppKey successfully to key \(keyIndex) for node \(unicastAddress)")
        self.meshRepository.onNetworkUpdated()
  ...
  @objc
  func selectModel(_ unicastAddress: Int, elementAddr: Int, modelId: Int, isSigModel:Bool, callback: @escaping RCTResponseSenderBlock){
    do {
      
      guard let meshNetwork = AppDelegate.shared.meshNetworkManager.meshNetwork else {
        throw NSError(domain: "MeshError", code: 404, userInfo: [NSLocalizedDescriptionKey: "Mesh network not found"])
      }
      
      guard let node = meshNetwork.node(withAddress: Address(unicastAddress)) else {
        throw NSError(domain: "MeshError", code: 404, userInfo: [NSLocalizedDescriptionKey: "Node not found"])
      }
  ...
  func getModelBoundKeys(_ callback: RCTResponseSenderBlock){
    guard let selectedModel = meshRepository.getSelectedModel() else {
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "Model is null"]), NSNull()])
      return
    }
  ...
  @objc
  func getPublicationSettings(_ unicastAddress: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.getPublicationSettings( unicastAddress:unicastAddress){ result, error in
      if let result = result {
        ...
        let errorMessage = error?.localizedDescription ?? "Unknown error"
        self.sendEvent(withName: "onPublicationUpdated", body: error?.localizedDescription)
        print("Failed to getPublicationSettings for node \(unicastAddress): \(errorMessage)")
        callback([["error": errorMessage], NSNull()])
      }
    }
  }
  ...
  @objc
  func setPublication(_ unicastAddress: Int, addressType : Int, appKeyIndex: Int, publishTtl: Int, publishAddress: String, publishPeriodInterval: Int, publishPeriodResolution: String, retransmitCount: Int, retransmitInterval: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.setPublicationSettings(unicastAddress: unicastAddress,
                                          ...
                                          retransmitInterval: retransmitInterval){ result, error in
      if let result = result {
        print(result)
        ...
        let errorMessage = error?.localizedDescription ?? "Unknown error"
        self.sendEvent(withName: "onPublicationUpdated", body: error?.localizedDescription)
        print("Failed to setPublication for node \(unicastAddress): \(errorMessage)")
        callback([NSNull(), NSNull()])
      }
    }
  }
  ...
  @objc
  func removePublication(_ unicastAddress: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.removePublicationSettings(unicastAddress: unicastAddress){ result, error in
      if let result = result {
        print(result)
  ...
  @objc
  func getSubscriptions(_ unicastAddress: Int, callback: @escaping RCTResponseSenderBlock){
    // Ensure the mesh network is available
    guard let meshNetwork = AppDelegate.shared.meshNetworkManager.meshNetwork else {
      print("No mesh network")
      let error = NSError(domain: "MeshModuleError", code: 0, userInfo: [NSLocalizedDescriptionKey: "No mesh network"])
      self.sendEvent(withName: "onSubscriptionReceived", body: error.localizedDescription)
      
      print("Failed to getSubscriptions for node \(unicastAddress): \(error.localizedDescription)")
    ...
    do {
      ...
      let subscriptionAddresses: [UInt16] = selectedModel.subscriptions
        ...
        .filter { $0 != 0xFFFF }
      self.sendEvent(withName: "onSubscriptionReceived", body: subscriptionAddresses)
  ...
  @objc
  func bindAppKey(_ unicastAddress: Int, appKeyIndex: Int, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.bindModelAppKey(unicastAddress: unicastAddress, keyIndex: appKeyIndex) { success, error in
      if success {
        ...
        print("Failed to bindAppKey for node \(unicastAddress): \(errorMessage)")
  ...
  @objc
  func unbindAppKey(_ appKeyIndex: Int, unicastAddress: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.unbindAppKey(unicastAddress:unicastAddress, keyIndex: appKeyIndex){ success, error in
      if success {
        ...
        print("Failed to onModelKeyUpdated for node \(unicastAddress): \(errorMessage)")
        callback([["error": errorMessage], NSNull()])
      }
    }
  }
  ...
  @objc
  func unsubscribe(_ subscriptionInx: Int, unicastAddress: Int, callback: @escaping RCTResponseSenderBlock){
    meshRepository.unsubscribe(unicastAddress:unicastAddress, subscriptionInx: subscriptionInx){ result, error in
      if result != nil {
        ...
        print("Failed to unsubscribe for node \(unicastAddress): \(errorMessage)")
        callback([["error": errorMessage], NSNull()])
      }
    }
  }
  ...
  @objc
  func generateGroupAddress(_ newGroupName: String, callback: @escaping RCTResponseSenderBlock) {
  ...
  func getGroups(_ callback: @escaping RCTResponseSenderBlock) {
    guard let meshNetwork = AppDelegate.shared.meshNetworkManager.meshNetwork else {
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "No mesh network"]), NSNull()])
      return
    }
  ...
  @objc
  func createNewGroup(_ groupName: String, groupAddress: Int, callback: @escaping RCTResponseSenderBlock) {
    guard let meshNetwork = AppDelegate.shared.meshNetworkManager.meshNetwork else {
      callback([NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey: "No mesh network"]), NSNull()])
      return
    }
    ...
    do {
      ...
      callback([NSNull(), newGroup.address.address])
      
    } catch {
      // Handle any error thrown during group creation
      callback([NSError(domain: "MeshModule", code: 2, userInfo: [NSLocalizedDescriptionKey: error.localizedDescription]), NSNull()])
    }
  }
  
  @objc
  func subscribeToExistingGroup(_ nodeUnicastAddress:Int, groupAddress: Int, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.subscribeToExistingGroup(nodeUnicastAddress:nodeUnicastAddress, groupAddress:groupAddress){ result, error in
      if result != nil {
        ...
        let errorMessage = error?.localizedDescription ?? "Unknown error"
        self.sendEvent(withName: "onSubscriptionAdded", body: error?.localizedDescription)
        
        print("Failed to onSubscriptionAdded to \(groupAddress): \(errorMessage)")
        callback([NSNull(), NSNull()])
      }
    }
  }
  ...
  @objc
  func subscribeToNewGroup(_ nodeUnicastAddress:Int, newGroupName:String, groupAddress: Int, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.subscribeToNewGroup(nodeUnicastAddress:nodeUnicastAddress,newGroupName: newGroupName, groupAddress:groupAddress){ result, error in
      if result != nil {
        ...
        let errorMessage = error?.localizedDescription ?? "Unknown error"
        self.sendEvent(withName: "onSubscriptionFailed", body: error?.localizedDescription)
  ...
  @objc
  func sendSensorGet(_ nodeUnicastAddress: Int, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.sendSensorGet(nodeUnicastAddress: nodeUnicastAddress )
    callback([NSNull(),  "Success"])
  }
  
  @objc
  func subscribe(_ nodeUnicastAddress:Int, addressToSuscribe: Int, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.subscribe(nodeUnicastAddress:nodeUnicastAddress, addressToSuscribe:addressToSuscribe){ result, error in
      if result != nil {
        ...
        print("Failed to onSubscriptionReceived to \(addressToSuscribe): \(errorMessage)")
        callback([NSNull(), NSNull()])
      }
    }
  }
  ...
  @objc
  func sendVendorModelMessage(_ nodeUnicastAddress:Int, opcode:Int, parameters: String, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.sendVendorModelMessage(nodeUnicastAddress:nodeUnicastAddress, opcode: opcode, parameters: parameters){ result, error in
      if result != nil {
        ...
        let errorMessage = error?.localizedDescription ?? "Unknown error"
        print("Failed to sendVendorModelMessage to \(nodeUnicastAddress): \(errorMessage)")
        callback([NSNull(), NSNull()])
      }
    }
  }
  ...
  func loadMeshNetwork(_ callback: @escaping RCTResponseSenderBlock) {
    do {
      ...
      let isNetworkLoaded = meshRepository.getMeshManager()?.isNetworkCreated ?? false
      if isNetworkLoaded, let network = meshRepository.getMeshManager()?.meshNetwork {
        NSLog("Mesh network loaded successfully.")
        // Emit the event to React Native
        self.sendEvent(withName: "onNetworkLoaded", body: ["networkName": network.meshName])
        AppDelegate.shared.updateLocalElements()
        // Return success via callback
        callback([NSNull(), ["success": true, "networkName": network.meshName]])
      } else {
      ...
      callback(["Error loading mesh network: \(error.localizedDescription)", NSNull()])
    }
  }
  
  @objc
  func editUnprovisionedNodeName(_ newNodeName: String, callback: @escaping RCTResponseSenderBlock) {
    let unprovisionedDevice = self.meshRepository.bleManager.selectedUnprovisionedNode.device;
    unprovisionedDevice.name = newNodeName;
    guard let provisioningManager = self.meshRepository.bleManager.provisioningManager else {
    ...
    let node: [String: Any] = [
      ...
      "numberOfElement": provisioningManager.provisioningCapabilities?.numberOfElements,
  ...
  @objc
  func editUnprovisionedNodeAddr(_ newNodeAddr: Int, callback: @escaping RCTResponseSenderBlock) {
    let unprovisionedDevice = self.meshRepository.bleManager.selectedUnprovisionedNode.device;
    guard let provisioningManager = self.meshRepository.bleManager.provisioningManager else {
      print("provisioning manager has not be initialized")
      callback([NSNull(), NSNull()])
  ...
  @objc
  func addProxyFilterAddresses(_ addresses: NSArray, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.addProxyFilterAddresses( addresses: addresses)
    callback([NSNull(), "success"])
  }
  
  @objc
  func removeProxyFilterAddress(_  address: String, callback: @escaping RCTResponseSenderBlock) {
  ...
  func setProxyFilterType(_ filterType: Int, callback: @escaping RCTResponseSenderBlock) {
    meshRepository.setProxyFilterType(filterType:filterType)
    callback([NSNull(), "success"])
  }
  ...
  func convertArrayToModelsList(models: NSArray) -> [SelectedModelInfo] {
    let swiftModels: [SelectedModelInfo] = models.compactMap { item in
      guard let dict = item as? [String: Any],
            ...
            let modelType = dict["modelType"] as? String else {
        return nil
      }
      return SelectedModelInfo(elementId: elementId, modelId: modelId, modelType: modelType)
    }
  ...
  @objc
  func bindAppKeyToModels(_ unicastAddress: Int, appKeyIndex: Int, models: NSArray, callback: @escaping RCTResponseSenderBlock) {
    // Convert NSArray (from React Native) to Swift array
    let swiftModels = convertArrayToModelsList(models:models);
    
    meshRepository.bindAppKeyToModels(unicastAddress: unicastAddress, keyIndex: appKeyIndex, models: swiftModels) { success, error in
      if success {
        callback([NSNull(), "Success"])
      } else {
        let errorMessage = error?.localizedDescription ?? "Unknown error"
  ...
  @objc
  func subscribeModels(_ unicastAddress: Int, groupAddress: String, models: NSArray, callback: @escaping RCTResponseSenderBlock) {
    // Convert NSArray (from React Native) to Swift array
    let swiftModels = convertArrayToModelsList(models:models);
    
    meshRepository.subscribeModels(unicastAddress: unicastAddress, groupAddress: groupAddress, models: swiftModels) { success, error in
      if success {
        callback([NSNull(), "Success"])
      } else {
        let errorMessage = error?.localizedDescription ?? "Unknown error"
  ...
  @objc
  func setPublicationToModelList(_ unicastAddress: Int, groupAddress: String, models: NSArray, appKeyIndex: Int, publishTtl: Int, publishPeriodInterval: Int, publishPeriodResolution: String, retransmitCount: Int, retransmitInterval: Int, callback: @escaping RCTResponseSenderBlock){
    
    // Convert NSArray (from React Native) to Swift array
    let swiftModels = convertArrayToModelsList(models:models);
    
    meshRepository.setPublicationSettingsToModelList(unicastAddress: unicastAddress,
  ...
  func getProvisioners(_ callback: @escaping RCTResponseSenderBlock) {
    ...
    for provisioner in provisioners {
      ...
      if let node = node {
        ...
        provDict["ttl"] = node.defaultTTL ?? MeshNetworkManager.instance.defaultTtl;
      ...
      for allocatedUnicastRange in provisioner.allocatedUnicastRange {
        ...
        allocatedUnicastRangeDict["lowAddress"] = allocatedUnicastRange.lowAddress;
        allocatedUnicastRangeDict["highAddress"] = allocatedUnicastRange.highAddress;
  ...
  @objc
  func editProvisionerName(_ provisionerUnicastAddress: Int, newName: String, callback: @escaping RCTResponseSenderBlock){
    guard let network = self.meshRepository.getMeshNetwork() else {
      print("no mesh network")
      callback([NSNull(), NSNull()]);
      return;
    ...
    let provisioners = network.provisioners
    let provisionerToEdit = provisioners.first(where: {$0.unicastAddress! == provisionerUnicastAddress})
  ...
  @objc
  func editProvisionerUnicastAddress(_ provisionerUnicastAddress: Int, newAddress: Int, callback: @escaping RCTResponseSenderBlock){
    ...
    do{
      if (provisionerToEdit != nil){
        let newAddress = UInt16(newAddress)
        try network.assign(unicastAddress: newAddress, for: provisionerToEdit!);
        AppDelegate.shared.meshNetworkManager.proxyFilter.add(address: newAddress)
      }
      AppDelegate.shared.meshNetworkManager.save()
      callback([NSNull(), "success"])
      ...
      var errorMessage = getErrorMessageFromProvisioner(error: error as! MeshNetworkError);
      let error = NSError(domain: "MeshModule", code: 0, userInfo: [NSLocalizedDescriptionKey:errorMessage])
      callback([NSNull(), error.localizedDescription])
    }
  ...
  @objc
  func editProvisionerTtl(_ provisionerUnicastAddress: Int, ttl: Int, callback: @escaping RCTResponseSenderBlock){
  ...
  @objc
  func editProvisionerUnicastRanges(_ provisionerUnicastAddress: Int, ranges: NSArray, callback: @escaping RCTResponseSenderBlock){
    ...
    do {
      let provisioners = network.provisioners
      if let provisionerToEdit = provisioners.first(where: {$0.unicastAddress! == provisionerUnicastAddress}){
        provisionerToEdit.deallocateUnicastAddressRange(AddressRange.allUnicastAddresses)
        ...
        for case let range as NSDictionary in ranges {
          if let low = range["lowAddress"] as? NSNumber,
            ...
            let newUnicastAddressRange = AddressRange(from: UInt16(low.uint16Value), to: UInt16(high.uint16Value))
        ...
        guard network.areRanges(addressesRange, availableForAllocationTo: provisionerToEdit) else {
  ...
  @objc
  func editProvisionerGroupRanges(_ provisionerUnicastAddress: Int, ranges: NSArray, callback: @escaping RCTResponseSenderBlock){
    ...
    do {
      let provisioners = network.provisioners
      if let provisionerToEdit = provisioners.first(where: {$0.unicastAddress! == provisionerUnicastAddress}){
        provisionerToEdit.deallocateGroupAddressRange(AddressRange.allUnicastAddresses)
        
        var addressesRange: [AddressRange] = [];
        for case let range as NSDictionary in ranges {
          if let low = range["lowAddress"] as? NSNumber,
             let high = range["highAddress"] as? NSNumber {
            let newAddressRange = AddressRange(from: UInt16(low.uint16Value), to: UInt16(high.uint16Value))
  ...
  func getErrorMessageFromProvisioner(error: MeshNetworkError) -> String{
    ...
    case .addressNotInAllocatedRange:
      errorMessage = "The Provisioner's address range is outside of its allocated range";
  ...
  @objc
  func editProvisionerScenesRanges(_ provisionerUnicastAddress: Int, ranges: NSArray, callback: @escaping RCTResponseSenderBlock){
      ...
      if let provisionerToEdit = provisioners.first(where: {$0.unicastAddress! == provisionerUnicastAddress}){
        ...
        for case let range as NSDictionary in ranges {
          if let low = range["firstScene"] as? NSNumber,
            ...
            let newSceneRange = SceneRange(from: UInt16(low.uint16Value), to: UInt16(high.uint16Value))
        ...
        guard network.areRanges(sceneRange, availableForAllocationTo: provisionerToEdit) else {
                throw MeshNetworkError.overlappingProvisionerRanges
        }
