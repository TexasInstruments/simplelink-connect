...
class QuickSetupNode: NSObject {
  ...
  func executeNext() {
    ...
    do {
      ...
      case .setPublication(_, to: let model):
        guard let address = model.parentElement?.parentNode?.unicastAddress else {
          fallthrough
        }
      ...
      taskResults.append((task: "Failed to send message \(task.message.opCode) to node \(node.unicastAddress)", success: false))
      
    }
  }
  
  func bindAppKeyToListModels(node: Node, applicationKey: ApplicationKey, models: [Model], completion: @escaping ([[String: Any]]) -> Void) {
    resetParams();
    self.node = node
  ...
  func subscribeToListModels(node: Node, group: Group, models: [Model], completion: @escaping ([[String: Any]]) -> Void) {
    resetParams();
  ...
  func setPublicationToListModels(node: Node, models: [Model], pubish:Publish, applicationKey: ApplicationKey, completion: @escaping ([[String: Any]]) -> Void) {
    resetParams();
...
extension QuickSetupNode : MeshNetworkDelegate {
  ...
  func meshNetworkManager(
    ...
    print("QuickSetupNode message from \(source): \(message), opCode: 0x\(String(format: "%02X", message.opCode))")
    ...
    if current >= 0 && current < tasks.count &&
      ...
      if let status = message as? ConfigStatusMessage {
        if status.isSuccess {
          ...
          taskResults.append((task: "\(tasks[current].title) Failed: \(status.message)", success: false))
        }
      }
      else {
        taskResults.append((task: tasks[current].title, success: true))
  ...
  private func meshNetworkManager(_ manager: MeshNetworkManager,
                                  ...
                                  error: Error) {
    inProgress = false
    print("Configuration Completed, message failed.")
  }
