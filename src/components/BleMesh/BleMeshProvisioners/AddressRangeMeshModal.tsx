...
import { AddressRange, meshStyles } from '../meshUtils';
import { FontAwesome5, MaterialCommunityIcons } from '@expo/vector-icons';

export interface AddressRangeModalData {
    title: string;
    currentValue: AddressRange[];
    onClickSave: (val: any) => void;
}

interface Props extends AddressRangeModalData {
    setModalVisible: any;
    visible: boolean;
}
export const AddressRangeMeshModal: React.FC<Props> = ({ visible, title, currentValue, setModalVisible, onClickSave }) => {

    const [low, setLow] = useState('');
    const [high, setHigh] = useState('');
    const [currentRanges, setCurrentRanges] = useState<AddressRange[]>([]);
    ...
    const handleAdd = () => {
        ...
        const isOverlapping = currentRanges.some(range =>
            newLow >= range.lowAddress && newHigh <= range.highAddress
        );
        ...
        setCurrentRanges(prev => [...prev, { lowAddress: newLow, highAddress: newHigh }]);
        setLow('');
        setHigh('');
    };
        ...
        <Modal
            ...
            <View style={[meshStyles.modelsCenteredView]}>
                <View style={styles.modalView}>
                    ...
                    <Text style={{ fontWeight: "500", fontSize: 16, marginBottom: 5 }}>Current Ranges</Text>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item, index }) => (
                                <View style={styles.rangeItem}>
                                    <Text style={styles.rangeText}>0x{item.lowAddress.toString(16).padStart(4, '0').toLocaleUpperCase()} - 0x{item.highAddress.toString(16).padStart(4, '0').toLocaleUpperCase()}</Text>

                                    <TouchableOpacity
                                        style={{ alignSelf: 'center', marginRight: 10 }}
                                        onPress={() => setCurrentRanges(prev => prev.filter(r => !(r.lowAddress === item.lowAddress && r.highAddress === item.highAddress)))}
                                    >
                    ...
                    {/* Add New Range */}
                    <Text style={{ fontWeight: "500", fontSize: 16, marginBottom: 5 }}>Add New Range</Text>
