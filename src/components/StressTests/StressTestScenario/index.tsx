import { View, StyleSheet, NativeEventEmitter, NativeModules, PermissionsAndroid, Platform, ScrollView } from 'react-native';
...
import TestStep from '../../StressTests/TestStep';
import RNRestart from 'react-native-restart';
import DeviceInfo from 'react-native-device-info';
import { hold, getCurrentDateTimeString, generateRandomBytes, LogLevel, DeviceName, Step, StepsIDs, TestResult, TestData, increaseProgress, increaseRound, setMTUSize, write, read, getNumOfPassed, TestParams, TEST_CASE, data_stream, simple_peripheral, sendEmail, NOTIFICATION_TIMEOUT } from '../testsUtils';
...
const StressTestScenario: React.FC = ({ }) => {
    ...
    const { testParametersContext, updateTestLogs, updateTestResults } = useStressTestContext();
    ...
    useEffect(() => {
        ...
        console.log("Test Parameters:", JSON.stringify(testParameters.current, null, 2));
        runTestScenario();
            ...
            if (!isBonded) {
                try {
                    ...
                    if (currentMainLoopNumber.current >= 3) {
                        // if this is the 3rd attempt, move to the next device,
                        addLog('Failed create bond after 3 attempts, Move to the next device', LogLevel.ERROR);
    ...
    async function stopTest() {
        ...
        updateTestResults(testData.current!)
        updateTestLogs(logs);
        updateRunning(false);
    ...
    async function executeWriteReadSimpleGatt(gattLoopNum: number, serviceUuid: string, readWriteChar: string) {
        ...
        if (readenData == randomBytes) {
            ...
            testResult.current!.test_pass = false;
            testResult.current!.error_message = "Read value does not match to the write value";
            setGattLoopFinished(true);
        }
                    ...
                    ({ value, peripheral, characteristic, service }) => {
                        ...
                        if (characteristic.toLocaleLowerCase().includes(notifChar.current.toLocaleLowerCase())) {
                            ...
                            // Got bigger notification
                                ...
                                testResult.current.got_expected_notification = false;
                                testResult.current!.test_pass = false;
                                testResult.current!.error_message = "Notification size does not match to expected size";
                                increaseProgress(steps.current, StepsIDs.gattTesting, 1);
                                setGattLoopFinished(true);
                                ...
                                notifTimeout.current = setTimeout(() => {
                                    addLog(`Received notification size ${notificationSize.current} / ${testParameters.current.expected_notifications_size} got timeout`, LogLevel.ERROR);
                                    testResult.current!.got_expected_notification = false;
                                    testResult.current!.test_pass = false;
                                    testResult.current!.error_message = `Received notification size ${notificationSize.current} / ${testParameters.current.expected_notifications_size} got timeout`;
