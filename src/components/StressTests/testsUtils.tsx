...
export function write(writeChar: string, serviceUuid: string, bytesToWrite: any, dataSize: number, addLog: any, periphralId: string, testResult: TestResult) {
    ...
    return new Promise<{ success: boolean, message?: string }>(async (resolve, reject) => {
        ...
        const timeoutPromise = new Promise<never>((_, rejectTimeout) =>
            setTimeout(() => rejectTimeout(new Error("Write operation timed out after 3 seconds")), TIMEOUT_MS)
        );

        try {
            // Race between the write operation and the timeout
            await Promise.race([
                BleManager.write(periphralId, serviceUuid, writeChar, bytesToWrite, dataSize),
                timeoutPromise
            ]);

            addLog('Written: ' + bytesToWrite, LogLevel.SUCCESS);
