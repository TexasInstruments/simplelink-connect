import { KeyboardAvoidingView, Platform, StyleSheet, ScrollView, TouchableOpacity, useWindowDimensions, Modal, TouchableWithoutFeedback, Keyboard, } from 'react-native';
...
import { getCurrentDateTimeString, TestParams, GATT_SERVICES, PHY_OPTIONS, TEST_CASE, TEST_CASES_OPTIONS, data_stream } from '../testsUtils';
import { downloadDataToLocalStorage } from '../../../services/DownloadFileUtils';
import Icon from 'react-native-vector-icons/MaterialIcons';
import { meshStyles } from '../../BleMesh/meshUtils';
...
const TestForm: React.FC<Props> = (
    ...
    const { testParametersContext, updateTestParams } = useStressTestContext();
    ...
    const [deviceToEditIdx, setDeviceIdx] = useState<number>(0);
    ...
    function handleEditDeviceName(newName: string) {
        const updatedDevices = devicesNameList.map((name, i) => i === deviceToEditIdx ? newName : name);
        setChanged(true);
        setDevicesNameList(updatedDevices);
    };

    function handleClickEdit(index: number) {
        const deviceToEdit = devicesNameList[index];
        setEditedName(deviceToEdit);
        setDeviceIdx(index);
                            ...
                            <View >
                                ...
                                <Dropdown
                                    ...
                                    onChange={(v: any) => {
                                        ...
                                        if (v.value === 'Simple GATT Profiles') {
                                            setTestCase('write & read')
                                        }
                                        else {
                                            setTestCase('write & receive notification')
                                        }
                                        setChanged(true);
    ...
    const EditModal = useMemo(() => (
        ...
        >
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={meshStyles.modelsCenteredView}>
                <View style={styles.modalView}>
                    ...
                    <View style={[styles.row, { marginTop: 10 }]}>
                        ...
                        <TouchableOpacity onPress={() => { handleEditDeviceName(editedName); setIsEditModalVisible(false); }}>
    ...
    return (
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 100 : 0} // Adjust as needed
        >
            <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
                ...
                <View style={{ flex: 1 }}>
                    ...
                    >
                        <ProgressStep label="Devices Info" nextBtnDisabled={isStepValid(0)} nextBtnStyle={{ ...styles.stepBtnStyle, borderWidth: isStepValid(0) ? 0 : 0.5 }}>
                        ...
                        <ProgressStep label="Main Loop Parameters" nextBtnDisabled={isStepValid(1)} nextBtnStyle={{ ...styles.stepBtnStyle, borderWidth: isStepValid(1) ? 0 : 0.5 }} previousBtnStyle={styles.stepBtnStyle}>
                            <View style={[styles.progressStepView]}>
                                {/* Pair and Bond */}
                                {Platform.OS === 'android' && (
                                    <View style={[styles.container, styles.row]}>
                        ...
                        <ProgressStep label="GATT Parameters" nextBtnDisabled={isStepValid(2)} finishBtnText="Run Test" onSubmit={runTest} nextBtnStyle={{ ...styles.stepBtnStyle, borderWidth: isStepValid(2) ? 0 : 0.5 }} previousBtnStyle={styles.stepBtnStyle}>
                            {GattParametersForm()}
                        </ProgressStep>
                    </ProgressSteps>
                </View>
